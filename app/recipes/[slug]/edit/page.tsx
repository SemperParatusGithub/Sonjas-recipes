...
export default function EditRecipePage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    params.then(async (p) => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          setPrepTimeMin(data.prepTimeMin?.toString() || '');
          setCookTimeMin(data.cookTimeMin?.toString() || '');
          setBakeTimeMin(data.bakeTimeMin?.toString() || '');
  ...
  if (error && !title) return (
    <div className="not-found">
      <h1>404</h1>
      <p>{error}</p>
      <Link href="/recipes" className="btn btn-primary">{t('viewAll')}</Link>
    </div>
  );
  ...
  return (
    <>
      ...
      <main className="recipe-form">
        <Link href={`/recipes/${slug}`} className="back-link">← {t('cancel')}</Link>
        <h1>{t('edit')}</h1>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-actions">
            <Link href={`/recipes/${slug}`} className="btn btn-secondary">{t('cancel')}</Link>
